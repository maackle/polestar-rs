    ...
    E: Clone + Eq + Hash + Debug + Display,
{
    ...
    write!(
        file,
        "{}",
        polestar_model_checker::diagram::to_dot(&graph, &[])
    )
    .unwrap();
    println!(
...
where
    ...
    M::Error: Debug,
{
    print_dot_state_diagram_mapped::<M, M::State, M::Action>(machine, initial, config, Some, Some)
}
    ...
    E: Clone + Eq + Hash + Debug + Display,
{
    println!(
        "{}",
        polestar_model_checker::diagram::to_dot(
